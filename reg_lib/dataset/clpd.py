...
class CLPD(data.Dataset):
    def __init__(self, config, is_train=True):
        ...
        with open(txt_file, 'r', encoding='utf-8') as file:
            ...
            for c in contents:
                ...
                label_str = c[-1]
                points = c[1:-1]
                self.labels.append({imgname: label_str, 'poly': points})
    ...
    def __getitem__(self, idx):
        ...
        img_name = list(self.labels[idx].keys())[0].split('/')[1]
        path = os.path.join(self.root,'CLPD_lp_area/'+img_name)
        img = cv2.imread(path)
